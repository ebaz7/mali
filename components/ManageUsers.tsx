...
import { UserPlus, Trash2, Shield, User as UserIcon, Download, CloudOff, Pencil, X, Save, Container, Camera, Send } from 'lucide-react';
...
const ManageUsers: React.FC = () => {
  ...
  const [formData, setFormData] = useState({ username: '', password: '', fullName: '', role: UserRole.USER, canManageTrade: false, avatar: '', telegramChatId: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); if (editingId) { const updatedUser: User = { id: editingId, ...formData }; await updateUser(updatedUser); setEditingId(null); } else { const user: User = { id: generateUUID(), ...formData }; await saveUser(user); } await loadUsers(); setFormData({ username: '', password: '', fullName: '', role: UserRole.USER, canManageTrade: false, avatar: '', telegramChatId: '' }); };
  const handleEditClick = (user: User) => { setEditingId(user.id); setFormData({ username: user.username, password: user.password, fullName: user.fullName, role: user.role, canManageTrade: user.canManageTrade || false, avatar: user.avatar || '', telegramChatId: user.telegramChatId || '' }); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const handleCancelEdit = () => { setEditingId(null); setFormData({ username: '', password: '', fullName: '', role: UserRole.USER, canManageTrade: false, avatar: '', telegramChatId: '' }); };
    ...
    <div className="space-y-8 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
          ...
          <div className="space-y-1 lg:col-span-2"><label className="text-sm text-gray-600">نقش کاربری</label><select value={formData.role} onChange={(e) => setFormData({...formData, role: e.target.value as UserRole})} className="w-full border rounded-lg px-3 py-2 text-sm bg-white"><option value={UserRole.USER}>کاربر عادی (فقط ثبت)</option><option value={UserRole.MANAGER}>مدیر (تایید درخواست)</option><option value={UserRole.FINANCIAL}>مدیر مالی</option><option value={UserRole.CEO}>مدیر عامل</option><option value={UserRole.ADMIN}>مدیر سیستم (دسترسی کامل)</option></select></div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-right"><thead className="bg-gray-5 text-gray-600"><tr><th className="px-6 py-3">تصویر</th><th className="px-6 py-3">نام و نام خانوادگی</th><th className="px-6 py-3">نام کاربری</th><th className="px-6 py-3">نقش</th><th className="px-6 py-3">دسترسی‌ها</th><th className="px-6 py-3 text-center">عملیات</th></tr></thead>
            <tbody className="divide-y divide-gray-100">{users.map((user) => (<tr key={user.id} className={`hover:bg-gray-50 transition-colors ${editingId === user.id ? 'bg-amber-50' : ''}`}><td className="px-6 py-4"><div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden">{user.avatar ? <img src={user.avatar} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-gray-400"><UserIcon size={20}/></div>}</div></td><td className="px-6 py-4 flex items-center gap-2">{user.fullName}</td><td className="px-6 py-4 font-mono text-gray-500">{user.username}</td><td className="px-6 py-4"><span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${user.role === UserRole.ADMIN ? 'bg-purple-50 text-purple-700 border-purple-200' : user.role === UserRole.CEO ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : user.role === UserRole.MANAGER ? 'bg-blue-50 text-blue-700 border-blue-200' : user.role === UserRole.FINANCIAL ? 'bg-green-50 text-green-700 border-green-200' : 'bg-gray-50 text-gray-700 border-gray-200'}`}>{user.role === UserRole.ADMIN && <Shield size={10} />}{user.role === UserRole.ADMIN ? 'مدیر سیستم' : user.role === UserRole.CEO ? 'مدیر عامل' : user.role === UserRole.FINANCIAL ? 'مدیر مالی' : user.role === UserRole.MANAGER ? 'مدیر داخلی' : 'کاربر عادی'}</span></td><td className="px-6 py-4 flex gap-1 flex-wrap">{user.canManageTrade && (<span className="flex items-center gap-1 text-[10px] bg-amber-50 text-amber-700 border border-amber-200 px-2 py-0.5 rounded w-fit"><Container size={10} /> بازرگانی</span>)}{user.telegramChatId && (<span className="flex items-center gap-1 text-[10px] bg-blue-50 text-blue-700 border border-blue-200 px-2 py-0.5 rounded w-fit"><Send size={10} /> تلگرام</span>)}</td><td className="px-6 py-4 text-center"><div className="flex items-center justify-center gap-2"><button onClick={() => handleEditClick(user)} className="text-amber-500 hover:text-amber-700 p-1 hover:bg-amber-50 rounded transition-colors" title="ویرایش / تغییر رمز"><Pencil size={16} /></button>{user.username !== 'admin' && (<button onClick={() => handleDeleteUser(user.id)} className="text-red-400 hover:text-red-600 p-1 hover:bg-red-50 rounded transition-colors" title="حذف کاربر"><Trash2 size={16} /></button>)}</div></td></tr>))}</tbody>
          </table>
        </div>
      </div>
    </div>
